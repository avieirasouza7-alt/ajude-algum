...
function nextPhase(species: WildlifeSpecies, current: AnimalPhase): AnimalPhase {
  ...
  if (current === "walk") {
    if (species === "deer") {
      // Cervos passam a maior parte do tempo pastando
      if (roll < 0.55) return "graze";
  ...
  if (current === "graze") return roll < 0.4 ? "walk" : roll < 0.75 ? "look" : "idle";
  if (current === "look") return roll < 0.65 ? "walk" : "graze";
  // idle
...
function phaseDuration(phase: AnimalPhase, species: WildlifeSpecies) {
  ...
  if (phase === "graze") return 4 + Math.random() * (species === "deer" ? 8 : 5);
  if (phase === "look") return 1.6 + Math.random() * 2.5;
...
export function PremiumWildlife({ species, radius, speed, offset }: WildlifeProps) {
  ...
  const pickWaypoint = (fromX: number, fromZ: number, now: number) => {
    ...
    for (let attempt = 0; attempt < 6; attempt++) {
      const step = 2.5 + Math.random() * 4.5;
      const heading = Math.random() * Math.PI * 2;
      const wx = fromX + Math.cos(heading) * step;
      const wz = fromZ + Math.sin(heading) * step;
    ...
    waypoint.current = { x: Math.cos(a) * r, z: Math.sin(a) * r };
    waypointSetAt.current = now;
  };
  ...
  useFrame((state, delta) => {
    ...
    if (walking || moveEase.current > 0.02) {
      ...
      if (walking && distance < 0.45) {
        ...
        phase.current = Math.random() < 0.6 ? "graze" : "look";
        phaseEndsAt.current = t + phaseDuration(phase.current, species);
      ...
      const linearSpeed = speed * radius * 1.15 * moveEase.current * arrive;
    ...
    if (head.current) {
      ...
      const look =
        phase.current === "look"
          ? /* Vira a cabeça em olhadas rápidas e pausadas, não pêndulo constante. */
            Math.round(Math.sin(t * 0.9 + offset) * 1.6) * 0.28
          : 0;
