...
function NightAtmosphereDome({ raining }: { raining: boolean }) {
  const material = useMemo(() => {
    const mat = new THREE.ShaderMaterial({
      ...
      transparent: true,
      uniforms: {
        zenith: { value: new THREE.Color(raining ? "#0a1018" : "#030712") },
        horizon: { value: new THREE.Color(raining ? "#1a2433" : "#1a2744") },
        glow: { value: new THREE.Color(raining ? "#243044" : "#2a3f6a") },
...
export function RealisticNightSky({
  ...
  const farStars = lowQuality ? 900 : isMobile ? 2200 : 4200;
  const midStars = lowQuality ? 0 : isMobile ? 700 : 1400;
  const milky = lowQuality ? 0 : isMobile ? 900 : 2200;

  return (
    <group>
      <color attach="background" args={[raining ? "#070b14" : "#02050e"]} />
      ...
      <ambientLight intensity={raining ? 0.14 : 0.18} color="#7f93c8" />
      <hemisphereLight args={["#1a2748", "#05080f", raining ? 0.22 : 0.35]} />
      <directionalLight
        position={moonPosition}
        intensity={raining ? 0.18 : 0.42}
        color="#c5d4ff"
        castShadow={!lowQuality && !raining}
        shadow-mapSize-width={isMobile ? 384 : 768}
        shadow-mapSize-height={isMobile ? 384 : 768}
