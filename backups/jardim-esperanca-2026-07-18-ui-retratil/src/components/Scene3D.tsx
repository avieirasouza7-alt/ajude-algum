...
function PerchedBird({
  ...
  useFrame((state) => {
    ...
    if (head.current) {
      // Olha para os lados em movimentos rápidos e pausados, como pássaro real
      head.current.rotation.y = Math.round(Math.sin(t * 0.8) * 2) * 0.4;
      head.current.rotation.x = Math.sin(t * 0.35) * 0.12;
    }
    if (body.current) {
      const hop = Math.max(0, Math.sin(t * 2.6)) ** 8;
      body.current.position.y = position[1] + hop * 0.045;
      body.current.rotation.z = Math.sin(t * 1.7) * 0.03;
    }
  });
  return (
    <group ref={body} position={position} rotation={[0, offset * 2.1, 0]} scale={0.75}>
      ...
      <mesh position={[0, 0.015, -0.13]} rotation={[Math.PI / 2.4, 0, 0]} scale={[1.5, 1, 0.28]}>
...
function HorizonTreeLine({ isNight, quality }: { isNight: boolean; quality: RenderQuality }) {
  ...
  const count = quality === "low" ? 70 : 120;
  const backCount = quality === "low" ? 50 : 90;
  const farCount = quality === "low" ? 40 : 70;
  /* Cor da névoa do horizonte — as árvores distantes desbotam nessa direção. */
  const hazeColor = useMemo(() => new THREE.Color(isNight ? "#101b26" : "#aabf9d"), [isNight]);
  ...
  return (
    <group>
      ...
      <instancedMesh ref={backRing} args={[undefined, undefined, backCount]}>
        ...
        <meshStandardMaterial
          color={isNight ? "#3a4a42" : "#ffffff"}
          map={leafTexture}
          roughness={1}
        />
      </instancedMesh>
      <instancedMesh ref={trunks} args={[undefined, undefined, count]}>
        <cylinderGeometry args={[0.7, 1.35, 2, quality === "low" ? 6 : 10, 3]} />
        <meshStandardMaterial
          color={isNight ? "#141a1a" : "#3f3024"}
          map={barkTexture}
          bumpMap={barkTexture}
          bumpScale={0.05}
          roughness={1}
      ...
      <instancedMesh ref={conifers} args={[undefined, undefined, count]}>
        <coneGeometry args={[0.85, 2, quality === "low" ? 7 : 11, 1]} />
        <meshPhysicalMaterial
    ...
    <>
      ...
      {isNight ? (
        ...
        <>
          ...
          />
          {/* Luz do dia mais quente e acolhedora — dourada no amanhecer/entardecer */}
          <ambientLight
            intensity={clearing ? 1.05 : twilight ? 0.68 : 0.78}
            color={clearing ? "#fff7e4" : twilight ? "#ffdcae" : "#fff1d6"}
          />
          <directionalLight
          ...
          <hemisphereLight
            args={[twilight ? "#f4c896" : "#c9ddff", "#6d8f58", twilight ? 0.42 : 0.5]}
          />
          {!isMobile && !lowQuality && (
            <>
      ...
      <HorizonHaze isNight={isNight} raining={raining} twilight={twilight} />
      ...
      <RealisticFlowerField low={lowQuality} isMobile={isMobile} />
      {/* Flores super detalhadas perto dos canteiros, balançando no vento */}
      <HeroFlowers low={lowQuality} />
      ...
      {!isNight && !raining && <SunGlow position={sunPosition} twilight={twilight} />}
