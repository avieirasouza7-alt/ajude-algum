...
type AnimalPhase = "walk" | "idle" | "graze" | "look";
...
function nextPhase(species: WildlifeSpecies, current: AnimalPhase): AnimalPhase {
  ...
  if (current === "walk") {
    if (roll < 0.4) return "idle";
    if (roll < 0.75) return "graze";
    return "look";
  }
  if (current === "graze") return roll < 0.55 ? "walk" : "idle";
  if (current === "look") return roll < 0.65 ? "walk" : "idle";
  // idle
  if (species === "fox") return roll < 0.7 ? "walk" : "look";
  return roll < 0.55 ? "walk" : "graze";
}

function phaseDuration(phase: AnimalPhase, species: WildlifeSpecies) {
  if (phase === "walk") return 4.5 + Math.random() * (species === "deer" ? 7 : 5);
  if (phase === "graze") return 3.5 + Math.random() * 5;
  if (phase === "look") return 2 + Math.random() * 2.5;
...
export function PremiumWildlife({ species, radius, speed, offset }: WildlifeProps) {
  ...
  // Initial tangent of the circular path; prevents a visible spin on spawn.
  ...
  useFrame((state, delta) => {
    ...
    if (t >= phaseEndsAt.current) {
      ...
      phaseEndsAt.current = t + phaseDuration(phase.current, species);
      // Occasionally reverse along the arc when blocked / restless
      if (Math.random() < 0.22) dirSign.current *= -1;
    }
    ...
    if (walking) {
      // Desired motion along a soft orbit, then collide-resolve so nothing clips
      const angular = speed * dt * 1.15 * dirSign.current;
      angle.current += angular;
      const desiredX = Math.cos(angle.current) * radius;
      const desiredZ = Math.sin(angle.current) * radius;
      // Blend toward desired point (keeps gait smooth when pushed)
      let nx = THREE.MathUtils.damp(pos.current.x, desiredX, 3.2, dt);
      let nz = THREE.MathUtils.damp(pos.current.z, desiredZ, 3.2, dt);
      ...
      if ((solid.hit || solid2.hit) && t >= collisionLockUntil.current) {
        // Reverse only once per collision. Without this lock a rabbit can
        // flip direction every frame and appear to vibrate in place.
        dirSign.current *= -1;
        collisionLockUntil.current = t + (species === "rabbit" ? 0.85 : 1.2);
        angle.current = Math.atan2(nz, nx) + dirSign.current * 0.1;
      }
      ...
      if (dx * dx + dz * dz > 1e-6) {
        ...
        facing.current += turn * Math.min(1, dt * (species === "rabbit" ? 7 : 4.5));
      }
      pos.current.x = nx;
      pos.current.z = nz;
    ...
    if (head.current) {
      const graze = phase.current === "graze" ? 0.55 : 0;
      const look = phase.current === "look" ? Math.sin(t * 1.4 + offset) * 0.35 : 0;
    ...
    if (tail.current) {
      const idleWag = phase.current === "idle" || phase.current === "look" ? 0.12 : 0.22;
      tail.current.rotation.y = Math.sin(gait * 0.72) * idleWag;
      tail.current.rotation.z =
        (species === "deer" ? 0.62 : 1.02) + Math.sin(gait * 0.5) * 0.08 * walkBlend.current;
    }
  });
