-- Conta usuários reais (auth.users), recria perfis faltantes e evita painel com 0 usuários
-- quando já existem campanhas (campaigns.user_id → auth.users, não exige profiles).
...
-- Backfill imediato (rodando como migration / SQL Editor com privilégios)
INSERT INTO public.profiles (id, full_name, avatar_url, account_status)
SELECT
  u.id,
  COALESCE(
