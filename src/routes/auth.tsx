...
function AuthPage() {
  ...
  const afterLoginPath = (resolvedUser?: import("@supabase/supabase-js").User | null) => {
    ...
    return !hasAcceptedTerms(u)
      ? "/aceitar-termos"
      : consumePublicAuthRedirect(redirect ?? "/painel");
  };

  useEffect(() => {
    if (finishingLogin || loading || busy) return;
    if (!user) return;
    const dest = !hasAcceptedTerms(user)
      ? "/aceitar-termos"
      : consumePublicAuthRedirect(redirect ?? "/painel");
    navigate({ to: dest });
  ...
  const handleSignup = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    toast.success(
      "Conta criada! Abra o link que enviamos no seu e-mail e depois faça login aqui.",
      {
        duration: 8000,
      },
    );
            ...
            <div className="rounded-2xl border border-border bg-card p-6 shadow-soft">
              <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-border bg-muted/30 p-4 text-sm">
                ...
                <span>
                  ...
                  , incluindo minha responsabilidade pelas campanhas, chave PIX, conteúdo e
                  divulgação.
                </span>
              </label>
