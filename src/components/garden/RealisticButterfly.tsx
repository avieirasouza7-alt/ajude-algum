...
export function RealisticButterfly({
  ...
  const flightClock = useRef(dna.offset);
  const hoverUntil = useRef(0);
  const nextHoverAt = useRef(4 + (dna.seed % 9));
  const pal = SPECIES_PALETTE[dna.species];
  ...
  useFrame((state, delta) => {
    ...
    if (!departing && now >= nextHoverAt.current) {
      hoverUntil.current = now + 0.7 + ((dna.seed * 17) % 100) / 85;
      nextHoverAt.current = hoverUntil.current + 5 + ((dna.seed * 31 + Math.floor(now)) % 8);
    }
    const hovering = !departing && now < hoverUntil.current;
    flightClock.current += delta * dna.orbitSpeed * (hovering ? 0.06 : 1);
    const t = flightClock.current;
    if (grp.current) {
      const flightRadius = dna.orbitRadius + leaving * 15;
      // Curvas sobrepostas evitam órbitas perfeitamente circulares. Durante
      // uma pausa, a borboleta paira quase imóvel sobre uma flor.
      let x =
        Math.cos(t) * flightRadius +
        Math.sin(t * 0.37 + dna.offset) * 0.42 +
        Math.sin(t * 1.73) * 0.12;
      let z =
        Math.sin(t) * flightRadius * 0.85 +
        Math.cos(t * 0.29 + dna.offset) * 0.36 +
        Math.cos(t * 1.41) * 0.1;
      let y =
        dna.height +
        Math.sin(t * 2.1) * dna.heightAmp +
        Math.sin(now * (hovering ? 2.2 : 5.4) + dna.offset) * (hovering ? 0.035 : 0.08) +
        leaving * 7;
      ...
      grp.current.rotation.z = THREE.MathUtils.damp(
        grp.current.rotation.z,
        hovering ? 0 : Math.sin(t * 1.5) * 0.09,
        5,
        delta,
      );
    }
    const flapRate = hovering ? 0.28 : 1;
    const flap =
      0.2 + Math.sin(state.clock.elapsedTime * dna.flapSpeed * flapRate + dna.offset) * 0.72;
